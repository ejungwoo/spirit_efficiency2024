...
(
  ...
  TString fMCName = "gen_n1333__132_0__0_1_2",
  //TString fMCName = "",
  TString fPathToData = "./data/", 
  TString fSupplePath = "/mnt/spirit/rawdata/misc/rawdataSupplement",
  TString fEventSelFileName = "config/EventArray_run2384_cut60_n271.txt"
)
{
  ...
  TString ggDataPathWithFormat = "/data/RB230064/production/ggNoise/ggNoise_%d.root";
  TString beamDataPathWithFormat = "/data/RB230064/jhpark/common/BeamData/beam_run%d.ridf.root";

  TString fMCDigiFile = TString("data/")+fMCName+".digi.root";

  std::vector<Int_t> eventArray;
  if (!fEventSelFileName.IsNull()) {
      ifstream eventSelectionFile(fEventSelFileName);
      int eventNo;
      while (eventSelectionFile>>eventNo)
          eventArray.push_back(eventNo);
  }
  ...
  TString out1 = fPathToData+"run"+sRunNo+"_s"+sSplitNo+".reco.exp.root";
  TString outc = fPathToData+"run"+sRunNo+"_s"+sSplitNo+".conc.exp.root";
  if(!fMCDigiFile.IsNull()) {
    out1 = fPathToData+"run"+sRunNo+"."+fMCName+".reco.embed.root";
    outc = fPathToData+"run"+sRunNo+"."+fMCName+".conc.embed.root";
  }
  ...
  if (!fEventSelFileName.IsNull())
      decoder -> SetEventList(eventArray);
  decoder -> SetEventID(start);
  ...
  embedTask -> SetEmbedFile(fMCDigiFile);
  ...
  smallOutput -> SetOutputFile(outc.Data());
  ...
  if(!fMCDigiFile.IsNull()) run -> AddTask(embedCorr);
